...
Feature: Syntax HRMS API End to End Workflow
  ...
  Scenario: Retrieving created employee
    ...
    And the retrieved employee ID at "employee[0].employee_id" matches the globally stored emplyee ID
    And the retrieved data at "employee" matches the data used to create an emplyee ID "employee[0].employee_id"
      | emp_firstname | emp_middle_name | emp_lastname | emp_birthday | emp_gender | emp_job_title | emp_status |
      | hakki         | ziya            | kimsavunur   | 2000-07-16   | Female     | Developer     | Worker     |
